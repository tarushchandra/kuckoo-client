...
import { usePostsLikedBy } from "@/hooks/queries/post-engagement";
import PostLikesModal from "./post-likes-modal";

interface MutualPostLikersProps {
  postId: string;
}

export default function PostLikes({ postId }: MutualPostLikersProps) {
  const postEngagement = usePostsLikedBy(postId);
  const [isLikedByModalOpen, setIsLikedByModalOpen] = useState(false);

  if (postEngagement === undefined) return <Skeleton className="w-32 h-4" />;

  if (!postEngagement || postEngagement?.likes?.length === 0)
    return <h1 className="text-sm font-semibold text-zinc-500">No likes</h1>;

  const firstThreeUsers = postEngagement.likes?.slice(0, 3);
  const remainingUsers = postEngagement.likes?.slice(3);
